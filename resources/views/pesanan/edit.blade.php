...
@section('page-subtitle', 'Perbarui data pesanan')
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="kontak" value="{{ old('kontak', $pesanan->kontak) }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat" rows="3" 
                                  ...
                                  placeholder="Alamat lengkap untuk pengiriman (opsional)">{{ old('alamat', $pesanan->alamat) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 border-b border-gray-200">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="jenis_produk" 
                            ...
                            @foreach(['Kaos','Polo','Seragam Sekolah','Jaket','Hoodie','Jersey Custom','Kemeja','Lainnya'] as $p)
                                <option value="{{ $p }}" {{ old('jenis_produk', $pesanan->jenis_produk)==$p ? 'selected' : '' }}>{{ $p }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="jumlah" id="jumlah" min="1" value="{{ old('jumlah', $pesanan->jumlah) }}"
                               class="w-full text-xs px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div>
                        ...
                        <input type="text" name="ukuran" value="{{ old('ukuran', $pesanan->ukuran) }}"
                               class="w-full text-xs px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div>
                        ...
                        <input type="text" name="warna" value="{{ old('warna', $pesanan->warna) }}"
                               class="w-full text-xs px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div class="md:col-span-2 space-y-4">
                        <div>
                            ...
                            <textarea name="spesifikasi" rows="3"
                                    class="w-full text-xs px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    placeholder="Contoh: Bahan Cotton Combed 30s, Sablon DTF, Bordir logo dada kiri">{{ old('spesifikasi', $pesanan->spesifikasi) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            @if($pesanan->foto_desain)
                                <div class="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                                    ...
                                    <img src="{{ asset('storage/'.$pesanan->foto_desain) }}" 
                                         class="h-32 w-auto rounded-lg shadow-md border border-gray-200"
                                         alt="Desain saat ini">
                                </div>
                            @endif
                            ...
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-500 transition-colors">
                                ...
                                <label for="foto_desain" class="cursor-pointer">
                                    ...
                                    <p class="text-xs text-gray-600" id="file_label">
                                        @if($pesanan->foto_desain)
                                            Klik untuk upload desain baru
                                        @else
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="tanggal_pesan"
                               value="{{ old('tanggal_pesan', $pesanan->tanggal_pesan ? \Carbon\Carbon::parse($pesanan->tanggal_pesan)->format('Y-m-d') : '') }}"
                               class="w-full text-xs px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="deadline"
                               value="{{ old('deadline', $pesanan->deadline ? \Carbon\Carbon::parse($pesanan->deadline)->format('Y-m-d') : '') }}"
                               class="w-full text-xs px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               required>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="harga_per_pcs" id="harga_per_pcs" min="0"
                                   value="{{ old('harga_per_pcs', $pesanan->harga_per_pcs) }}"
                        ...
                        <div class="bg-gradient-to-r from-blue-50 to-blue-100 border-2 border-blue-200 rounded-xl p-6">
                            <div class="flex items-center justify-between flex-wrap gap-4">
                                <div>
                                    ...
                                    <p class="text-base font-bold text-blue-700" id="total_harga_display">
                                        Rp {{ number_format($pesanan->total_harga, 0, ',', '.') }}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-600 mb-1">Sisa Pembayaran</p>
                                    <p class="text-sm font-bold" id="sisa_pembayaran">
                                        @if($pesanan->total_harga - $pesanan->dp > 0)
                                            <span class="text-orange-600">Rp {{ number_format($pesanan->total_harga - $pesanan->dp, 0, ',', '.') }}</span>
                                        @else
            ...
            <div class="p-6 border-b border-gray-200">
                ...
                <textarea name="keterangan" rows="3"
                          class="w-full text-xs px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          placeholder="Catatan khusus untuk pesanan ini...">{{ old('keterangan', $pesanan->keterangan) }}</textarea>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const sisaDisplay = document.getElementById('sisa_pembayaran');
    ...
    function calculateTotal() {
        ...
        if (sisa > 0) {
            sisaDisplay.innerHTML = '<span class="text-orange-600">Rp ' + sisa.toLocaleString('id-ID') + '</span>';
        } else {
            sisaDisplay.innerHTML = '<span class="text-green-600">Lunas</span>';
        }
    }
    
    if (hargaPerPcs) hargaPerPcs.addEventListener('input', calculateTotal);
    if (jumlahInput) jumlahInput.addEventListener('input', calculateTotal);
    if (dpInput) dpInput.addEventListener('input', calculateTotal);
